import React from 'react';
import { CheckCircle2, Loader2, Circle } from 'lucide-react';
import { useEnvironmentStore } from '../../../stores/environmentStore';

interface ProcessChecklistProps {
  progress: number;
}

export const ProcessChecklist: React.FC<ProcessChecklistProps> = ({ progress }) => {
  const { dashboardData } = useEnvironmentStore();
  const ds = dashboardData?.dataset;
  const fileName = ds?.name || 'kharghar_survey.csv';
  const obsCount = ds?.observations || 300;

  const items = [
    {
      id: 'file',
      title: 'File uploaded',
      desc: fileName,
      threshold: 15
    },
    {
      id: 'parse',
      title: 'Parsing data',
      desc: `${obsCount} rows · 12 columns`,
      threshold: 40
    },
    {
      id: 'coords',
      title: 'Validating coordinates',
      desc: 'Latitude & Longitude verified',
      threshold: 65
    },
    {
      id: 'params',
      title: 'Detecting parameters',
      desc: 'PM2.5 · PM10 · CO₂ · Temp · Humidity',
      threshold: 85
    },
    {
      id: 'stats',
      title: 'Preparing dataset',
      desc: progress >= 100 ? 'Statistics ready' : 'Calculating statistics...',
      threshold: 100
    }
  ];

  return (
    <div className="space-y-3.5 select-none font-sans">
      {items.map((item) => {
        const isDone = progress >= item.threshold;
        const isCurrent = progress < item.threshold && (progress >= item.threshold - 25 || item.id === 'file');

        return (
          <div 
            key={item.id}
            className={`flex items-start space-x-3.5 p-3 rounded-2xl transition-all duration-300 ${
              isDone 
                ? 'bg-[#EAF7EE]/60 border border-[#C3E8CC]/50' 
                : isCurrent 
                ? 'bg-[#FFF0E5]/60 border border-[#F47A24]/30 shadow-xs' 
                : 'bg-[#FAF3EA]/40 border border-transparent opacity-60'
            }`}
          >
            {/* Status Icon */}
            <div className="pt-0.5 shrink-0">
              {isDone ? (
                <CheckCircle2 className="w-5 h-5 text-[#3FA66B]" />
              ) : isCurrent ? (
                <Loader2 className="w-5 h-5 text-[#F47A24] animate-spin" />
              ) : (
                <Circle className="w-5 h-5 text-[#C0B4A8]" />
              )}
            </div>

            {/* Content */}
            <div className="flex-1 min-w-0">
              <div className={`text-[13.5px] font-extrabold tracking-tight ${
                isDone ? 'text-[#2B211C]' : isCurrent ? 'text-[#F47A24]' : 'text-[#8C827A]'
              }`}>
                {item.title}
              </div>
              <div className="text-[11.5px] font-medium text-[#8C827A] mt-0.5">
                {item.desc}
              </div>
            </div>
          </div>
        );
      })}
    </div>
  );
};
